...
use super::tools::{
    aggregate_tool_calls, parse_moonlight_tool_calls, strip_moonlight_tool_markers,
};
...
pub async fn handle_streaming(
    ...
    let transformed_stream = futures_util::stream::unfold(
        ...
        move |(
            ...
            first_bytes_time,
            is_first_chunk,
            mut moonlight_tool_call_buffer,
            mut moonlight_in_tool_call,
        )| async move {
                ...
                while let Some(pos) = buffer.find('\n') {
                    ...
                    let openai_chunk_result: StreamChunkResult = match provider_type {
                        ...
                        ProviderType::Moonlight => {
                            ...
                            let _is_final_chunk = if let Ok(chunk) =
                            ...
                            match serde_json::from_str::<OpenAIStreamChunk>(data) {
                                Ok(mut openai_chunk) => {
                                    ...
                                    for choice in &mut openai_chunk.choices {
                                        let content_has_markers = choice
                                            .delta
                                            .content
                                            .as_ref()
                                        ...
                                        if !moonlight_in_tool_call && content_has_markers {
                                            tracing::debug!(
                                                "Moonlight: starting tool call buffer, content={:?}",
                                                choice.delta.content
                                            );
                                            moonlight_in_tool_call = true;
                                            moonlight_tool_call_buffer =
                                                choice.delta.content.clone();
                                            has_content = true;
                                        } else if moonlight_in_tool_call {
                                            if let Some(ref content) = choice.delta.content {
                                                tracing::debug!(
                                                    "Moonlight: buffering content: {}",
                                                    content
                                                );
                                            ...
                                            if finish_reason.is_some() {
                                                tracing::debug!(
                                                    "Moonlight: got finish_reason, parsing buffered content"
                                                );
                                                if let Some(ref buf) = moonlight_tool_call_buffer {
                                                    let parsed = parse_moonlight_tool_calls(buf);
                                                    tracing::debug!(
                                                        "Moonlight: parsed {} tool_calls from buffer",
                                                        parsed.len()
                                                    );
                                                ...
                                                moonlight_in_tool_call = false;
                                                moonlight_tool_call_buffer = None;
                                            }
                                        } else if let Some(ref content) = choice.delta.content {
                                            counter.add_delta(content);
                                            has_content = true;
                                        }
                                    }

                                    if !has_content && !moonlight_in_tool_call {
                                    ...
                                    if !tool_call_chunks.is_empty() {
                                        ...
                                        StreamChunkResult::Single(OpenAIStreamChunk {
                                            id: id.clone(),
                ...
                if !yielded_chunks.is_empty() {
                    ...
                    return Some((
                        ...
                        (
                            ...
                            first_bytes_time,
                            false,
                            moonlight_tool_call_buffer.clone(),
                            moonlight_in_tool_call,
                        ),
                    ));
                }
